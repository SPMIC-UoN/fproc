...
class T1wFatWater(Module):
    ...
    def process(self):
        water_in = self.single_inimg("../fsort/t1w_water", "t1w_water.nii.gz", warn=False)
        ...
        if inphase and opphase:
            ...
            water_calc = inphase.save_derived(water_calc, self.outfile("t1w_water_calc.nii.gz"))
            fat_calc = (inphase.data - opphase.data) / 2
            fat_calc = inphase.save_derived(fat_calc, self.outfile("t1w_fat_calc.nii.gz"))
        ...
        if water_in and fat_in:
            ...
            self.no_data("No input for fat/water calculation; no scanner fat/water either")
...
MODULES = [
    ...
    segmentations.TotalSeg(
        name="totalseg_t1w", src_dir="../fproc/t1w_fat_water", water_glob="t1w_water.nii.gz", fat_glob="t1w_fat.nii.gz"
    ),
    segmentations.TotalSeg(
        name="totalseg_t2w", src_dir="../fsort/t2w_fs", water_glob="t2w_fs.nii.gz", fat_glob=None
    ),
    segmentations.TotalSeg(
        name="totalseg_dwi", src_dir="../fsort/dwi_adc", water_glob="dwi_adc.nii.gz", fat_glob=None
    ),
