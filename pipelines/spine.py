...
class Regrid(Module):
    ...
    def regrid(self, imgs):
        ...
        LOG.info(
            f" - Desired A-P dimension {pa_size}, actual {new_shape[pa_axis]}, cropping {crop_voxels} each side"
        )
        crop = [0, 0, 0, 1]
...
class SpineSeg(Module):
    ...
    def _reorient(self, data, ax_labels, reverse=False):
        ...
        if not reverse:
            ...
            if not lr_flip:
                data = np.flip(data, lr_axis)
            if not pa_flip:
                data = np.flip(data, pa_axis)
            if not is_flip:
                data = np.flip(data, is_axis)
            return data
        else:
    ...
    def _save_coronal_data(self, data, name):
        ...
        data_res = self._regrid(
            data, size=(self.AP, self.LR), batch_dims=1
        )  # [IS, AP, LR]
        data_res = self._reorient(
            np.transpose(data_res, (1, 2, 0)), self.ax_labels, reverse=True
        )
    ...
    def process(self):
        ...
        model.load_state_dict(
            torch.load("weights/resnet34_deepV3.pth", map_location=torch.device("cpu"))
        )
        model.eval()
        ...
        LOG.info(
            f" - Coronal mask: {np.min(coronal_mask)}, {np.max(coronal_mask)}: {coronal_mask.shape}"
        )
        coronal_mask_normed = (coronal_mask / np.max(coronal_mask) * 255).astype(
            np.uint8
        )
        ...
        contours, _ = cv2.findContours(
            coronal_mask_normed, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        for idx, contour in enumerate(contours):
            ...
            if area > 1000:
                ...
                cv2.fillPoly(
                    coronal_mask_clean, pts=selected_contours, color=255
                )  # [IS, 256, 3]
                LOG.info(
                    f" - Filled coronal mask vox={np.count_nonzero(coronal_mask_clean)}"
                )
        LOG.info(
            f" - Selected {len(selected_contours)} coronal contours with area > 1000"
        )
        ...
        LOG.info(
            f" - Interpolated coronal mask vox={np.count_nonzero(coronal_mask_res)}"
        )
        ...
        for i in range(top, bottom):
            ...
            # Back to original resolution
            inp = self._regrid(
                inp, size=(fat_tensor.shape[0], fat_tensor.shape[1])
            )  # [PA, LR]
        ...
        plt.axis("off")
        plt.savefig(
            self.outfile("coronal_overlay.png"),
            bbox_inches="tight",
            pad_inches=0,
            dpi=1000,
        )
