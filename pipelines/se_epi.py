...
class Nifti(Module):
    ...
    def process(self):
        ...
        for mask in ("cortex", "medulla"):
            masks = list(
                glob.glob(os.path.join(datadir, "masks", f"{subjid}_*{mask}*.nii*"))
            )
            if not masks:
                ...
                if not mask_l or not mask_r:
                    LOG.warn(
                        f"Could not identify L/R {mask} masks for {subjid}: {masks} - ignoring"
                    )
                else:
                    ...
                    combined_data = np.logical_or(left.data, right.data).astype(
                        np.int32
                    )
                    right.save_derived(combined_data, self.outfile(f"{mask}.nii.gz"))
                    overlay_masks[mask] = ImageFile(
                        self.outfile(f"{mask}.nii.gz"), warn_json=False
                    )
            elif len(masks) == 1:
                LOG.info(
                    f" - Single {mask} mask found for {subjid}: {masks[0]} - splitting L/R"
                )
                ...
                LOG.warn(
                    f"Wrong number of {mask} masks for {subjid}: {masks} - ignoring"
                )
        ...
        for name, fname, subdir, matcher_glob in [
            ...
            if not imgs:
                ...
                if len(imgs) > 1:
                    LOG.warn(
                        f"Multiple {name}s found for {subjid}: {imgs} - choosing first"
                    )
                img = imgs[0]
                ...
                for mask in ("cortex", "medulla"):
                    if mask in overlay_masks:
                        self.lightbox(
                            map,
                            overlay_masks[mask],
                            self.outfile(f"{fname}_{mask}_lightbox"),
                        )
