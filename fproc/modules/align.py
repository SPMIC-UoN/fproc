...
class FlirtAlignOnly(Module):
    ...
    def process(self):
        ...
        if (
            run_flirt
            and "weight_mask" in self.kwargs
            and "weight_mask_dir" in self.kwargs
        ):
            weight_mask = self.single_inimg(
                self.kwargs["weight_mask_dir"],
                self.kwargs["weight_mask"],
                src=self.OUTPUT,
            )
            if weight_mask is None:
                ...
                for idx in range(self.kwargs.get("weight_mask_dil", 0)):
                    weight_mask_data = scipy.ndimage.binary_dilation(
                        weight_mask_data, structure=np.ones((5, 5, 5))
                    )
                weight_mask = weight_mask.save_derived(
                    weight_mask_data, self.outfile("weight_mask.nii.gz")
                )
        ...
        for img in imgs_to_align:
            ...
            if run_flirt:
                ...
                ref_img_res = self.resample(
                    ref_img, img, is_roi=False, allow_rotated=True
                )
                flirt_opts = {
                ...
                if weight_mask is not None:
                    ...
                    weight_mask_res = self.resample(
                        weight_mask, img, is_roi=True, allow_rotated=True
                    )
                    weight_mask_res_data = weight_mask_res.get_fdata()
                    vol_frac_before = (
                        np.count_nonzero(weight_mask.data) / weight_mask.data.size
                    )
                    vol_frac_after = (
                        np.count_nonzero(weight_mask_res_data)
                ...
                flirt_result = fsl.flirt(
                    img.nii, ref_img_res, out=fsl.LOAD, omat=fsl.LOAD, **flirt_opts
                )
                    ...
                    for extra_img in extra_imgs:
                        if extra_img.affine_matches(img):
                            ...
                            if mat is not None:
                                ...
                                if (
                                    "kidney" in extra_img.fname
                                    or "seg" in extra_img.fname
                                ):
                                    # FIXME below is only for segmentations...
                                    img_align = nib.Nifti1Image(
                                        img_align.get_fdata() > 0.5,
                                        img_align.header.get_best_affine(),
                                        img_align.header,
                                    )
                            else:
