...
def parse_args():
    ...
    parser.add_argument("--pipeline", required=True, help="Name of python module containing pipeline config, or path to .py file")
    parser.add_argument("--input", required=True, help="Input directory - may be relative to pipeline.BASE")
    parser.add_argument("--flatten", action="store_true", help="Run fproc-flatten to flatten images")
...
def main():
    ...
    for cohort_name, cohort_prefix, subjids in pipeline.COHORTS:
        ...
        for file_id, paths in pipeline.OUTFILES.items():
            run_fproc_combine(
                input_dir,
                os.path.join(output_base, f"{pipeline.NAME}_{cohort_prefix}{file_id}_{dateout}.csv"),
                paths,
                subjids=subjids
            )
    ...
    if options.flatten:
        ...
        run_fproc_flatten(
            input_dir,
            os.path.join(output_base, f"imgs_{dateout}")
        )
